...
encode_msg_endpoint(#{} = M, Bin, TrUserData) ->
    ...
    case M of
	  ...
	  if TrF5 == [] -> B4;
	     true -> e_field_endpoint_tags(TrF5, B4, TrUserData)
	  end;
      _ -> B4
    end.
...
dfp_read_field_def_endpoint(<<34, Rest/binary>>, Z1, Z2,
			    F@_1, F@_2, F@_3, F@_4, F@_5, TrUserData) ->
    d_field_endpoint_port_name(Rest, Z1, Z2, F@_1, F@_2,
			       F@_3, F@_4, F@_5, TrUserData);
dfp_read_field_def_endpoint(<<42, Rest/binary>>, Z1, Z2,
			    F@_1, F@_2, F@_3, F@_4, F@_5, TrUserData) ->
...
dg_read_field_def_endpoint(<<0:1, X:7, Rest/binary>>, N,
    ...
    case Key of
	  ...
	  d_field_endpoint_port(Rest, 0, 0, F@_1, F@_2, F@_3,
				F@_4, F@_5, TrUserData);
      34 ->
	  d_field_endpoint_port_name(Rest, 0, 0, F@_1, F@_2, F@_3,
				     F@_4, F@_5, TrUserData);
      42 ->
...
d_field_endpoint_port_name(<<1:1, X:7, Rest/binary>>, N,
			   Acc, F@_1, F@_2, F@_3, F@_4, F@_5, TrUserData)
    when N < 57 ->
    d_field_endpoint_port_name(Rest, N + 7, X bsl N + Acc,
			       F@_1, F@_2, F@_3, F@_4, F@_5, TrUserData);
d_field_endpoint_port_name(<<0:1, X:7, Rest/binary>>, N,
...
merge_msg_endpoint(PMsg, NMsg, TrUserData) ->
    ...
    S5 = case {PMsg, NMsg} of
	   {_, #{port_name := NFport_name}} ->
	       S4#{port_name => NFport_name};
	   {#{port_name := PFport_name}, _} ->
	       S4#{port_name => PFport_name};
	   _ -> S4
	 end,
    case {PMsg, NMsg} of
      {#{tags := PFtags}, #{tags := NFtags}} ->
	  S5#{tags => 'erlang_++'(PFtags, NFtags, TrUserData)};
...
v_msg_endpoint(#{} = M, Path, TrUserData) ->
    ...
    case M of
      #{port_name := F4} ->
	  v_type_string(F4, [port_name | Path], TrUserData);
      _ -> ok
    end,
    case M of
      #{tags := F5} ->
...
get_msg_defs() ->
    [{{msg, service},
	 ...
	 occurrence => optional, opts => []},
       #{name => port_name, fnum => 4, rnum => 5,
	 type => string, occurrence => optional, opts => []},
       #{name => tags, fnum => 5, rnum => 6, type => string,
